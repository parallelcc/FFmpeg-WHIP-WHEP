...
static int whip_write_header(AVFormatContext *s)
{
    ...
    for (unsigned int i = 0, ssrc = av_get_random_seed(); i < s->nb_streams; i++, ssrc++) {
        ...
        switch (st->codecpar->codec_id) {
        case AV_CODEC_ID_AV1:
            pinit.obuPacketization = RTC_OBU_PACKETIZED_TEMPORAL_UNIT;
            if (rtcSetAV1Packetizer(whip->tracks[i], &pinit) < 0) {
                av_log(s, AV_LOG_ERROR, "Failed to set AV1 packetizer\n");
                ret = AVERROR_EXTERNAL;
                goto fail;
            }
            break;
